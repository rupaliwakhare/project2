...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let allProducts = products; // ✅ IMPORTANT FIX
  ...
  function applyFilters() {
    ...
    if (filters.color.size)
      filtered = filtered.filter(p =>
        p.color.some(c => filters.color.has(c))
      );
  ...
  document.addEventListener("change", e => {
    ...
    if (e.target.checked)
      filters[type].add(e.target.value);
    else
      filters[type].delete(e.target.value);
